...
from refedez.build.flare import FlareProvision, nvidia_flare_from_configuration
from refedez.build.nix import NixMachine, Closure
...
from refedez.common.errors import ProjectAlreadyRunningError
from refedez.common.machines import Machine
from refedez.common.project import ReFedEzProject
from refedez.common.state import MachineRunning, Running
...
def build_closures_for_all_machines(project: ReFedEzProject) -> List[Closure]:
    builds = []
    with tqdm(
...
def patch_provision_with_closures(provision: FlareProvision, closures: List[Closure]):
    with tqdm(
        total=len(closures),
        desc="✂️ Patching provisioned files so they run on the generated ReFedEz environment...",
        bar_format="{l_bar}{bar} | {n_fmt}/{total_fmt} [{elapsed}<{remaining}]",
    ) as pbar:
...
def execute(refedez_project: ReFedEzProject):
    state = refedez_project.project_state
    if state.is_running:
        raise ProjectAlreadyRunningError()
    flare = nvidia_flare_from_configuration(refedez_project)
    provision = flare.provision()
    closures = build_closures_for_all_machines(project=refedez_project)
...
def start(path: str) -> None:
    ...
    project = ReFedEzProject(config=configuration)
    execute(project)
