...
from refedez.common.state import Running
from refedez.lib.backends import Backend
from refedez.lib.backends.numpy import __create_numpy_job
from refedez.lib.backends.torch import __create_torch_job
from refedez.lib.backends.tensorflow import __create_tf_job
...
def create_job(job_config: JobConfiguration, output_path: str) -> str:
    match job_config.type:
        ...
        case Backend.TENSORFLOW:
            return __create_tf_job(job_config, output_path)


def execute_job_config(job_config: JobConfiguration):
    configuration = get_refedez_config_from_yaml(path=job_config.refedez_config)
