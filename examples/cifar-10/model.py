...
from refedez.lib.refedez import Client, Server, Federated
...
class CIFAR10Federated(FederatedTorch):
    ...
    def __init__(
        ...
        self.device = device_def or (
            device("cuda:0") if cuda.is_available() else device("cpu")
        )
    ...
    def _create_datasets(self, index_file: Optional[str] = None):
        ...
        # set local model save paths if not set and if refedez provides working_dir attribute
        ...
        for k in list(local_sd.keys()):
            if k in new_state:
                ...
                if isinstance(val, np.ndarray):
                    t = as_tensor(val, device=self.device)
                elif isinstance(val, Tensor):
                    t = val.to(self.device)
                else:
                    # try cast
                    t = tensor(val, device=self.device)
                # reshape if needed
                try:
        ...
        for epoch in range(self.aggregation_epochs):
            ...
            if self.writer:
                ...
                self.writer.add_scalar(
                    "train_loss", avg_loss / max(1, epoch_len), global_step
                )
    ...
    def train_step(self, learning_rate=1.0) -> Dict[str, Tensor]:
        """
        Returns the updated model weights as CPU tensors (state_dict).
        The refedez framework may call train_step with server weights.
        """
    ...
    def validate(
        self, incoming_weights: Dict[str, Any], index_file: Optional[str] = None
    ) -> Dict[str, float]:
        """
